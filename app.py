...
Servier VORANIGO Glioma Research Platform
"""
...
COLORS = {
    'High Vora Usage': '#00C850',
    'High Vora User': '#00C850',
    'Low Vora Usage': '#E8A838',
    'Low Vora User': '#E8A838',
    'Non Vora User': '#8898C8',
    'Non Vora Usage': '#8898C8',
    'On Target': '#C8102E',
    'Off target': '#8898C8',
    'Co-locs': '#E8A838',
    'Academic': '#00B4B4',
...
class DataEngine:
    ...
    def _load_raw_file(self, file_obj, label="file"):
        """Load ATU or PET raw file with auto-detected header."""
        try:
    ...
    def _load_segment_file(self, file_obj, label="segment"):
        """Load a segment file (standard 7-column format)."""
        try:
    ...
    def load_from_paths(self, atu_path, pet_path, hl_path=None, atu_pet_path=None, rep_path=None):
        """Load from file paths (for default project files)."""
        ...
        if self.segment_hl is not None:
            ...
            if 'Wave Number' in seg_hl.columns:
                ...
                seg_hl_latest = seg_hl_latest[['User Id', 'Segment Value', 'Wave Number']].rename(
                    columns={'Segment Value': 'Vora_Usage_Segment', 'Wave Number': 'Latest_Wave_HL'})
    ...
    def get_summary_stats(self):
        ...
        if self.overlap_df is not None and 'Vora_Usage_Segment' in self.overlap_df.columns:
            seg_counts = self.overlap_df['Vora_Usage_Segment'].value_counts().to_dict()
            stats['vora_segments'] = seg_counts
...
class QuestionClassifier:
    ...
    SEGMENT_RULES = {
        ...
        'Brand Awareness & Familiarity': [
            ...
            r'VORANIGO', r'vorasidenib', r'Temodar', r'temozolomide', r'Tibsovo',
        ...
        'Access, Reimbursement & Support': [
            ...
            r'ServierONE', r'patient.*support', r'bridge.*program', r'PAP',
...
def render_sidebar():
    with st.sidebar:
        ...
        if data_mode == "Upload Custom Files":
            st.markdown("**ATU Raw File** (2 quarters)")
            uploaded['atu'] = st.file_uploader("ATU", type=['xlsx', 'csv'], label_visibility="collapsed")
            st.markdown("**PET Raw File** (3 quarters)")
            uploaded['pet'] = st.file_uploader("PET", type=['xlsx', 'csv'], label_visibility="collapsed")
            st.markdown("**Segment: High/Low/No User** *(optional)*")
            uploaded['hl'] = st.file_uploader("HL Segment", type=['xlsx'], label_visibility="collapsed")
            st.markdown("**Segment: ATU+PET Perception** *(optional)*")
            uploaded['atu_pet'] = st.file_uploader("ATU PET Segment", type=['xlsx'], label_visibility="collapsed")
            st.markdown("**Segment: Rep Attributes** *(optional)*")
            uploaded['rep'] = st.file_uploader("Rep Segment", type=['xlsx'], label_visibility="collapsed")
        ...
        <b style='color: #F0F4FF;'>VORANIGO® Glioma Program</b><br>
        Servier Internal Analytics<br>
...
def view_executive_overview(engine: DataEngine):
    ...
    <div class='hero-banner'>
        <div class='hero-badge'>VORANIGO® GLIOMA PROGRAM · FY26</div>
    ...
    with col_right:
        # Voranigo Usage Segments
        if 'vora_segments' in stats and stats['vora_segments']:
            seg_labels = list(stats['vora_segments'].keys())
            seg_vals = list(stats['vora_segments'].values())
            fig_seg = make_donut(seg_vals, seg_labels, 'VORANIGO Usage Segments<br>(Overlapping HCPs)', COLORS)
...
def view_overlap_analysis(engine: DataEngine):
    ...
    with fc1:
        seg_options = ['All'] + sorted(df['Vora_Usage_Segment'].dropna().unique().tolist()) if 'Vora_Usage_Segment' in df.columns else ['All']
        seg_filter = st.selectbox("Vora Usage Segment", seg_options)
    with fc2:
    ...
    # Apply filters
    if seg_filter != 'All' and 'Vora_Usage_Segment' in df.columns:
        df = df[df['Vora_Usage_Segment'] == seg_filter]
    ...
    for col in ['ATU_Quarters', 'PET_Quarters', 'ATU_Target_Type', 'PET_Target_Type',
                'Vora_Usage_Segment', 'ATU_PET_Segment', 'Rep_Interaction_Segment',
...
def view_segment_intelligence(engine: DataEngine):
    ...
    tabs = st.tabs(["VORANIGO Usage Segments", "ATU+PET Combined Segments", "Rep Interaction Segments"])
    ...
    with tabs[0]:
        if engine.segment_hl is None:
            ...
            with col1:
                ...
                fig = make_bar(seg_counts, 'Segment', 'Count', 'VORANIGO Usage Distribution',
    ...
    with tabs[1]:
        if engine.segment_atu_pet is None:
            ...
            💡 <b>Segment Interpretation:</b> These segments combine VORANIGO usage level (from ATU) 
...
def view_question_classifier(engine: DataEngine):
    ...
    if input_mode == "Upload Survey File":
        ...
        atu_questions = [
            ...
            "How familiar are you with each of the following as a treatment for IDH-mutant astrocytoma or oligodendroglioma? Including Temozolomide, PCV regimen, VORANIGO, ivosidenib",
            ...
            "How would you rate VORANIGO, Temozolomide+RT, and Radiation therapy alone on each attribute including PFS, OS, adverse events, hepatic toxicity?",
            "What percent of the time do you dispense VORANIGO to grade 2 IDH-mutant astrocytoma or oligodendroglioma patients via internal vs external specialty pharmacy?",
            "How much impact do insurance coverage, NCCN guidelines, reimbursement level have on your decision to prescribe VORANIGO?",
            "Which of the following are the main barriers to prescribing VORANIGO for IDH-mutant astrocytoma or oligodendroglioma patients?",
            "How familiar are you with the ServierONE Patient Support Program?",
            "Which ServierONE patient support services are you aware of - Commercial Co-Pay Program, Bridge Program, Patient Assistance Program, QuickStart?",
            "How often do patients ask about VORANIGO as a treatment option for IDH-mutant astrocytoma or oligodendroglioma?",
            "To what extent does patient inquiry influence your decision to prescribe or recommend VORANIGO?",
        ...
        pet_questions = [
            "How much time (in minutes) did you spend with the Servier rep during your most recent interaction?",
            "During your most recent interaction with the Servier representative, what proportion of time was spent discussing Disease State Education vs VORANIGO product?",
            ...
            "Please select the type of interaction you had with Servier sales rep - In-person, Video conference, Tele-conference",
            "Which of the following topics were discussed during your most recent interaction with the Servier representative? (Efficacy, Safety, Dosing, Patient types, Access, NCCN Guidelines, Molecular testing)",
            "What are the primary messages you recall hearing during your most recent interaction with the Servier sales rep discussing VORANIGO for Gliomas?",
            "Which VORANIGO sales messages do you specifically recall hearing - VORANIGO indicated for Grade 2 patients 12+, first treatment in 20+ years, dual inhibitor targeting mIDH1 and mIDH2?",
            "Did the Servier sales rep specifically ask you to prescribe VORANIGO for Gliomas?",
            "Prior to your most recent interaction with Servier sales representative, please rate your likelihood to prescribe VORANIGO for Gliomas",
            "How likely are you to increase prescribing VORANIGO for Gliomas based on your most recent interaction with the Servier representative?",
            "Based on your most recent interaction with the Servier sales rep, please indicate the change in your perception of VORANIGO across Efficacy, Safety, Dosing, Access attributes",
            "Please indicate the importance of rep performance attributes - How prepared, organized, knowledgeable, engaging was the Servier rep?",
            "How frequently do you discuss Gliomas with Servier sales reps?",
            "Servier reps have provided me with useful materials that help me educate my patients about VORANIGO",
            "Servier reps have been supportive in ensuring that my patients can get access to VORANIGO",
            "Which ServierONE patient support services are you aware of - Commercial Co-Pay, Bridge Program, PAP, QuickStart?",
            "Please indicate via which channels you have received information regarding VORANIGO - Online ads, Email, Medical journals, Conferences, Social media",
            "Please rate the online ad you recently saw about VORANIGO across motivating, believable, clear, relevant dimensions",
            ...
            "VORANIGO is indicated for treatment of patients 12 years and older with Grade 2 astrocytoma or oligodendroglioma with IDH1 or IDH2 mutation - is this message differentiated, believable, motivating?",
            "VORANIGO is the first new treatment in more than 20 years for Grade 2 mIDH glioma - how differentiated and motivating is this message?",
        ]
...
def view_temporal_trends(engine: DataEngine):
    ...
    with col1:
        ...
        if engine.segment_hl is not None and 'Wave Number' in engine.segment_hl.columns:
            st.markdown("### VORANIGO Usage Segment by Wave")
    ...
    def load_project_files():
        ...
        try:
            engine.load_from_paths(
                ...
                hl_path=project / 'High_vs_Low_vs_No_User_ATU_PET_Segment.xlsx',
                atu_pet_path=project / 'ATU_Usage_and_PET_Perception.xlsx',
                rep_path=project / 'Rep_Driven_Attributes_ATU.xlsx',
            )
        except Exception as e:
            engine.errors.append(str(e))
        return engine

    if data_mode == "Use Project Files":
        engine = load_project_files()
    else:
        # Upload mode
