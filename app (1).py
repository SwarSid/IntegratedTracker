...
Servier Glioma Research Platform
"""
...
class DataEngine:
    ...
    def _mask_brand_str(cls, text: str) -> str:
        """Replace all brand-name variants in a string with PRODUCT GLIOMA."""
        if not isinstance(text, str):
            return text
        return cls._BRAND_PATTERN.sub(cls._MASK, text)
    ...
    def _mask_df(cls, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Mask brand names everywhere in a DataFrame:
          ...
          2. All object-dtype (string) columns — every cell value
        ...
        # 2. All string cell values across every object column
...
def view_question_classifier(engine: DataEngine):
    ...
    if input_mode == "Upload Survey File":
        ...
        pet_questions = [
            ...
            "During your most recent interaction with the Servier representative, what proportion of time was spent discussing Disease State Education vs Glioma product?",
            ...
            "What are the primary messages you recall hearing during your most recent interaction with the Servier sales rep discussing Glioma for Gliomas?",
            ...
            "Did the Servier sales rep specifically ask you to prescribe Glioma for Gliomas?",
            "Prior to your most recent interaction with Servier sales representative, please rate your likelihood to prescribe Glioma for Gliomas",
            "How likely are you to increase prescribing Glioma for Gliomas based on your most recent interaction with the Servier representative?",
            "Based on your most recent interaction with the Servier sales rep, please indicate the change in your perception of Glioma across Efficacy, Safety, Dosing, Access attributes",
            ...
            "Servier reps have provided me with useful materials that help me educate my patients about Glioma",
            "Servier reps have been supportive in ensuring that my patients can get access to Glioma",
...
def main():
    ...
    def load_project_files():
        engine = DataEngine()
        # Search for project files in multiple candidate locations
        candidate_dirs = [
            Path('/mnt/project'),
            Path('.'),
        ...
        if project is None:
            engine.errors.append(
                "Project files not found. Please use 'Upload Custom Files' mode in the sidebar "
                "to manually upload your ATU and PET raw files."
            )
            return engine

        def _safe_path(p):
            """Return path if exists, else None (suppresses FileNotFoundError)."""
        ...
        try:
            engine.load_from_paths(
                ...
                hl_path=_safe_path(project / 'High_vs_Low_vs_No_User_ATU_PET_Segment.xlsx'),
                atu_pet_path=_safe_path(project / 'ATU_Usage_and_PET_Perception.xlsx'),
                rep_path=_safe_path(project / 'Rep_Driven_Attributes_ATU.xlsx'),
            )
        except Exception as e:
